...
func TestTaskA(t *testing.T) {
	var values = []struct {
		...
		{"Верно", 2, 1.04, 2.04, 0.5, []float64{0.520316235595818, 0.7177865337405192, 0.5878711701956065}},
		{"Верно", 4, 9.034, 9.039, 0.02, []float64{0.1113898676606079, 0.1498764947240627, 0.19861893241865275}},
	}
...
func TestTaskB(t *testing.T) {
	var values = []struct {
		...
		{"Верно", 1.321, []float64{4.5, 3.222, 5.00}, []float64{0.3376564479793544, 0.5664507473034334, 0.3965790442838673}},
		{"Верно", 15, []float64{0, 3.7}, []float64{0.10947371185997536, 0.12849119680379323}},
	}
	for _, value := range values {
		t.Run(value.text, func(t *testing.T) {
