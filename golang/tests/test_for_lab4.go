...
func Testing_lab4() {
	testingOutputY(&testing.T{})
	testingTaskA(&testing.T{})
	testingTaskB(&testing.T{})
}
func testingOutputY(t *testing.T) {
	const const_a = 2
	const const_b = 3
	result_y := labs.ReturnY(0.11, const_a, const_b)
...
func testingTaskA(t *testing.T) {
	y_list_true := []float64{1.581565621572023,
		...
		1.6372494546297607,
		1.6540889552598357}
	y_list_output := labs.TaskA(2, 3, 0.11, 0.36, 0.05)
...
func testingTaskB(t *testing.T) {
	y_list_true := []float64{1.5766843704639988,
		1.6208710138786773,
		1.650716645162541,
		1.6707225698630865,
		1.7060955449869923}
	y_list_output := labs.TaskB(2, 3, []float64{0.08, 0.26, 0.35, 0.41, 0.53})
	for i, y := range y_list_output {
		if y != y_list_true[i] {
			t.Errorf("The function was calculated incorrectly")
		}
